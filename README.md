...
**Throw on first use** (rare paths only: REPL, heap-dump, agent-proxy relay, `claude gateway`): `Bun.generateHeapSnapshot`, `Bun.Transpiler`, `Bun.listen`, `Bun.serve`, `Bun.connect`, `Bun.build`, `Bun.Image`, and every member of the `Bun.ant` namespace (next paragraph).
...
A separate `AUDIT_INERT_BUN` map in `update.sh` whitelists `Bun.*` symbols that appear **only** as inert string-literal content the bundle emits verbatim (e.g. `Bun.stdin` inside a scaffolded hook-handler template). Those are deliberately **not** shimmed — source-replacing them would corrupt the emitted template — and the audit accepts them only while every occurrence still matches a recorded context fingerprint. When such a symbol later grows a *distinct* executable form that the general alternation cannot rewrite without also hitting the inert one (v2.1.269 added `Bun.stdin.stream().getReader()`, next to the inert `.text()` template and the VS-Code-only `edit-hook` path), it gets a **dedicated** source-replace in `launcher.js` (`BUN_STDIN_RE` → `globalThis.__bunShimStdin`), recorded in `AUDIT_DEDICATED_BUN`; the audit then requires the occurrence count to equal the sum of the inert and dedicated contexts, so a genuinely new form still re-trips it.
...
- **`test/bunfile-shim.test.js`** covers the `Bun.file` shim (input forms, lazy construction, positioned `fd` reads, MIME mapping, `writer()`, `stream()`, and the deliberate `slice()` throw.
- **`test/toml-shim.test.js`** covers the `Bun.TOML` shim (including the documented limits of the date round-trip.
- **`test/spawn-shim.test.js`** covers the non-PTY `Bun.spawn` shim (POSIX-only): the exact shapes of the bundle's call sites (rg probe, bg-pty-host breadcrumb, spare pool), stream readers, fd ownership, signal/ENOENT degradation, and a guard that fails loudly if an `unref()`d child drains the event loop before the report runs.
- **`test/plugin-shim.test.js`** verifies that loading `plugin-shim.js` via `node --require` correctly sets up `globalThis.Bun` with the shimmed API surface (namespace presence, YAML round-trip, file construction, hash, inert values, throw-on-use symbols, and Bun.ant members).
- **`test/extract-modulegraph.test.js`** pins the `.bun` module-table layout that `extract-modulegraph.py` was reverse-engineered against: a synthetic section built in the test round-trips byte-exact through the extractor, the entry follows the trailer word rather than table position, and every format violation the real dump could develop overnight (header count, trailer, table stride, entry range, name root, non-JS entry, pointer bounds) exits 2 without writing a manifest.
- **`test/modulegraph-loader.test.js`** covers the loader: every resolve rule (virtual `file:///$bunfs/root/` URLs, `bun:*` refusal, builtins and bare externals from graph modules, relative imports refused), every load rewrite (`Bun.X` to `__bunShim.X` outside string literals, `import.meta.require`, asset and addon path literals), and an end-to-end child process that registers the hooks with `module.registerHooks()` and imports a synthetic entry, because only Node itself can prove it accepts source served for URLs that exist nowhere on disk.
...
- **v2.1.247/248**: first new Bun API since the module-graph era: `Bun.build` (Bun's bundler) arrived with the rollout-gated plugin *hooks modules* feature, which bundles a plugin's `hooks.ts` at runtime; the release audit blocked two nightlies on it, as designed. Left as a throws-stub: the single call site catches and wraps into its own `HooksError`, so the feature degrades with its designed message under Node. The same sweep audited the growth of already-shimmed symbols the release audit deliberately accepts: four unguarded `Bun.serve` sites now exist (hooks-module loopback tool server, two preview/reachability servers, the gateway listener), all feature-path-only, so the throws-stub stays correct; the new `Bun.spawn` shapes (BunFile in `stdio`, `terminal:`) were already covered. One real semantic gap surfaced: `Bun.which(cmd, {PATH})` (sandbox PATH filtering) would silently resolve against the process PATH, because npm `which` spells the option `path`; the shim now maps it. **v2.1.249/250** then added `require()` of graph *JS modules* (356 direct `import.meta.require("...")` literals, some inside genuine import cycles like chunk-ns0ekkj0 <-> chunk-3w64c04v). The loader serves them through Node's `require(esm)` at the VIRTUAL path (same hooks, same module-map entry as `import`, measured instance-identical). Hoisting the literals into static imports was tried first and is documented as WRONG: it reorders graph evaluation and the bundle's esbuild lazy-init pattern depends on requires running at their original position. Because Node refuses mid-cycle requires (`ERR_REQUIRE_CYCLE_MODULE`) while Bun hands out the not-yet-finished namespace, and 2.1.250's cycles surface in several shapes (top-level, immediate reads, transitively through a deferred require's subgraph), the js case always returns ONE stable lazy namespace proxy per module: evaluation on first property access, mid-cycle reads yield `undefined` without memoizing the failure (later reads heal, matching var-bound live bindings), and the namespace shape comes statically from the source's `export{...}` clause so re-export copying works mid-cycle.
- **v2.1.266**: `Bun.Image`, Bun's built-in sharp-compatible image processor (`new Bun.Image(buf).resize().jpeg()/.png().toBuffer()/.metadata()`), replaced the previous image path. One call site, the image-attach / clipboard-paste compression pipeline, reached lazily through an async loader, so nothing at startup or in non-image use touches it. Throws-stub per the stub-until-it-fires policy: a real shim would be `sharp` (its API already matches the call sites) but that is a heavy native libvips dep with SIGILL risk on this pre-POPCNT CPU. Image attachment therefore degrades with a clear error until someone wants it enough to add sharp.
- **v2.1.258**: the tool registry chunk started capturing *values* at module top level, `wLn = import.meta.require("/$bunfs/root/chunk-….js").ArtifactTool` (same for Workflow, Monitor, ProposeSkills, EndConversation), while each of those chunks statically imports the registry back. Bun evaluates the required chunk right there against the registry's in-flight namespace (plain cyclic-evaluation semantics), so the tool exists; Node refuses to even *link* a synchronous require graph that touches a module currently evaluating (`Cannot import Module … in a cycle`), the lazy proxy hands out `undefined`, and a captured `undefined` never heals. Five tools vanished from the registry and startup died with `Cannot read properties of undefined (reading 'name')`; the release audit and the `--version`/`--help` smoke test cannot see it because the registry is only built on the way into a session. The refusal is Node policy, not a V8 limit: given the in-flight job as the dependency, V8 evaluates the module against the partial namespace exactly as inside a static-import cycle (measured in a three-module repro and pinned by `test/modulegraph-loader.test.js`). `installRequireCycleTolerance()` in the loader therefore wraps the ESM loader's public `getOrCreateModuleJob()` and returns the cached job on precisely that error. That needs `--expose-internals`, which `NODE_OPTIONS` rejects, so `launcher.js` re-executes itself with the flag when it is missing (~1 s extra on a Core2Duo); a wrapper that passes `node --expose-internals launcher.js` skips the extra process.
- **v2.1.251**: embedded text assets ship zstd-compressed (magic-byte check at the call site, passthrough for uncompressed), visible in the loader census as text 166 -> 72 / file 4 -> 101. `Bun.zstdDecompress`/`zstdDecompressSync` arrived for exactly that and got real `node:zlib` implementations rather than stubs, per the shim policy for cheap, well-defined APIs.
...
- **v2.1.269**: `Bun.stdin` grew a second, executable form. Since v2.1.141 it had appeared only as inert `new Response(Bun.stdin.stream()).text()` template content and rode the `AUDIT_INERT_BUN` carve-out; a `.text()` call also lived in the VS-Code-only `claude edit-hook` path, textually identical to the template and thus silently carried along. 2.1.269 added a bounded stdin reader `Bun.stdin.stream().getReader()` inside a CLI command, a genuinely executable use, which diverged the count (3 occurrences, 2 matching the `.text()` fingerprint) and correctly tripped the audit. It could not join the general `BUN_SHIM_RE` alternation: that blind substitution would also rewrite the `Bun.stdin` inside the emitted scaffolding template (mid-string, so the quote-lookbehind does not protect it) and corrupt the file a user gets. Fix: a real `globalThis.__bunShimStdin` (`.stream()` = `Readable.toWeb(process.stdin)`) and a **dedicated** `BUN_STDIN_RE` scoped to the `.getReader()` form only, applied in the loader after the alternation; `AUDIT_DEDICATED_BUN` records it and the audit now requires the occurrence count to equal inert + dedicated. The template and the edit-hook `.text()` path stay verbatim (edit-hook remains unreached under this Node deployment, as before).
